    ...
    def provision_tenant(
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if "already" in error_str or "exists" in error_str or "duplicate" in error_str:
                        # User exists, we'll need to get their ID
                        # For now, we'll create the user with a temporary approach
                        # In production, you might want to use invite_user instead
                        logger.info(f"User {admin_email} already exists, will link to tenant")
                        # We'll need the user_id - for existing users, this requires
                        # either storing it or using invite_user which returns the user
                        # For now, raise an error asking to use invite_user for existing users
                ...
                logger.error(f"Failed to create/invite admin user: {e}")
            ...
            try:
                ...
                raise ProvisioningError(f"Failed to link admin user: {str(e)}")
            ...
            self._rollback(tenant_id, user_id, bucket_created)
            raise ProvisioningError(f"Unexpected error: {str(e)}")
    ...
    def _rollback(
        ...
        if tenant_id and user_id:
            try:
                ...
                logger.error(f"Failed to rollback tenant_users: {e}")
        ...
        if bucket_created and tenant_id:
            try:
                ...
                logger.error(f"Failed to rollback storage bucket: {e}")
        ...
        if tenant_id:
            try:
                ...
                logger.error(f"Failed to rollback tenant: {e}")
