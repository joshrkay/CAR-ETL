...
def test_flag_toggle():
    ...
    if response.status_code == 404:
        print(f"   [OK] Endpoint returned 404 (feature disabled)")
    else:
    ...
    if is_enabled:
        print(f"   [OK] Flag correctly evaluated as enabled")
        ...
        print(f"   [OK] Service confirms flag is enabled")
        print(f"   [NOTE] Endpoint test requires cache invalidation or wait for TTL")
    else:
        print(f"   [FAIL] Flag should be enabled but isn't")
    ...
    if not is_enabled:
        ...
        print(f"   [FAIL] Flag should be disabled but isn't")
