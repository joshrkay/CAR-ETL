...
def test_tenant_isolation():
    ...
    try:
        ...
        if not service_key or not service_key.startswith(('eyJ', 'sb_')):
            print(f"   [WARNING] Service key format may be incorrect.")
            print(f"   [INFO] Service key should be a JWT (starts with 'eyJ') or Supabase key.")
            print(f"   [INFO] Current key starts with: {service_key[:10] if service_key else 'None'}...")
            print(f"   [INFO] Get your service_role key from: Supabase Dashboard → Settings → API")
        ...
        if "Bearer token" in error_msg or "authentication" in error_msg.lower():
            ...
            print(f"   [INFO] The service_role key may be incorrect or missing.")
            print(f"   [INFO] Service key format check:")
            service_key = config.supabase_service_key
            if service_key:
                if service_key.startswith('eyJ'):
                    print(f"      ✓ Key appears to be a JWT (correct format)")
                elif service_key.startswith('sb_'):
                    print(f"      ⚠ Key appears to be a publishable key (may not work for admin operations)")
                    print(f"      [INFO] You need the 'service_role' key, not the 'anon' or 'publishable' key")
                else:
                    print(f"      ⚠ Key format is unusual: starts with '{service_key[:10]}...'")
                print(f"      [INFO] Key length: {len(service_key)} characters")
            else:
                print(f"      ✗ Service key is not set")
            ...
            try:
                ...
                print(f"   1. Go to: Supabase Dashboard → Settings → API")
            print(f"   2. Copy the 'service_role' key (not 'anon' or 'publishable')")
            print(f"   3. Set it as SUPABASE_SERVICE_KEY environment variable")
            print(f"   4. The service_role key should be a JWT token starting with 'eyJ'")
    ...
    try:
        ...
        print(f"   [OK] Linked User A to Tenant A")
        ...
        print(f"   [OK] Linked User B to Tenant B")
    ...
    try:
        ...
        print(f"   [OK] Got access token for User A")
        ...
        print(f"   [OK] Got access token for User B")
        
    except Exception as e:
        print(f"   [ERROR] Failed to get access tokens: {e}")
        print(f"   [INFO] Make sure users were created successfully and passwords are correct.")
        return False
    ...
    print(f"   [OK] Created client for User A (anon_key + JWT)")
    print(f"   [OK] Created client for User B (anon_key + JWT)")
    ...
    try:
        ...
        if tenant_a_id in tenant_ids_a and tenant_b_id not in tenant_ids_a:
            print(f"      [PASS] User A can only see Tenant A")
        else:
            print(f"      [FAIL] User A can see Tenant B! Violation of tenant isolation!")
        ...
        if tenant_b_id in tenant_ids_b and tenant_a_id not in tenant_ids_b:
            print(f"      [PASS] User B can only see Tenant B")
        else:
            print(f"      [FAIL] User B can see Tenant A! Violation of tenant isolation!")
        ...
        if all(tid == tenant_a_id for tid in user_tenant_ids_a):
            print(f"      [PASS] User A can only see Tenant A's users")
        else:
            print(f"      [FAIL] User A can see Tenant B's users! Violation of tenant isolation!")
        ...
        if all(tid == tenant_b_id for tid in user_tenant_ids_b):
            print(f"      [PASS] User B can only see Tenant B's users")
        else:
            print(f"      [FAIL] User B can see Tenant A's users! Violation of tenant isolation!")
        ...
        try:
            ...
            if len(result_a_tenant_b.data) == 0:
                print(f"      [PASS] User A cannot query Tenant B by ID (RLS blocked)")
            else:
                print(f"      [FAIL] User A can query Tenant B by ID! Violation of tenant isolation!")
                return False
        except Exception as e:
        ...
        try:
            ...
            if len(result_b_tenant_a.data) == 0:
                print(f"      [PASS] User B cannot query Tenant A by ID (RLS blocked)")
            else:
                print(f"      [FAIL] User B can query Tenant A by ID! Violation of tenant isolation!")
                return False
        except Exception as e:
            # Some Supabase clients may raise an error instead of returning empty
