"""Test JWT generation with provided credentials."""
import json
import sys
...
def main() -> None:
    """Main test function."""
    print("=" * 60)
    print("JWT Generation Test with Provided Credentials")
    print("=" * 60)
    print()
    
    # Credentials
    domain = "dev-khx88c3lu7wz2dxx.us.auth0.com"
    client_id = "eTcYVJTcXQiLf5eAdpPzRDH6mKT9pXF0"
    client_secret = "x3Pzmgm5rjnCgoILrpV2CANLVc_73sT4M02bzDlElFB4Hk4Tm540iF1mOUvz4PfO"
    ...
    # Test 1: Management API token
    print("=" * 60)
    print("Test 1: Management API Token")
    print("=" * 60)
    
    management_audience = f"https://{domain}/api/v2"
    jwks_uri = f"https://{domain}/.well-known/jwks.json"
    
    print(f"[INFO] Attempting to get Management API token...")
    print(f"[INFO] Audience: {management_audience}")
    
    token = get_token(domain, client_id, client_secret, management_audience)
    ...
    if token:
        ...
        # Verify token
        print("\n" + "=" * 60)
        print("Verifying Token with JWKS")
        print("=" * 60)
        
        payload = decode_jwt_token(token, jwks_uri, management_audience)
        ...
        print("\n[WARN] Could not get Management API token")
        print("   This is expected if Management API is not enabled")
        print("   or the client is not authorized for Management API")
    
    # Test 2: CAR API token (if API resource exists)
    print("\n" + "=" * 60)
    print("Test 2: CAR API Resource Token")
    print("=" * 60)
    
    api_audience = "https://api.car-platform.com"
    
    print(f"[INFO] Attempting to get CAR API token...")
    print(f"[INFO] Audience: {api_audience}")
    print("[INFO] Note: This will only work if:")
    print("   1. CAR API resource exists in Auth0")
    print("   2. This client is authorized for CAR API")
    ...
    if token:
        ...
        print("\n[WARN] Could not get CAR API token")
        print("   This is expected if:")
        print("   1. CAR API resource doesn't exist yet")
        print("   2. This client is not authorized for CAR API")
        print("   3. This client is configured for Management API only")
    
    # Summary
    print("\n" + "=" * 60)
    print("Test Summary")
    print("=" * 60)
    print("\n[INFO] JWKS Endpoint Status:")
    try:
        jwks_response = httpx.get(jwks_uri, timeout=10.0)
        if jwks_response.status_code == 200:
            jwks = jwks_response.json()
            print(f"   [SUCCESS] JWKS accessible: {len(jwks.get('keys', []))} keys found")
            print(f"   Algorithm: {jwks.get('keys', [{}])[0].get('alg', 'N/A')}")
        else:
            print(f"   [ERROR] JWKS returned status: {jwks_response.status_code}")
    except Exception as e:
        print(f"   [ERROR] Could not access JWKS: {e}")
    
    print("\n[INFO] Next Steps:")
    print("   1. If Management API token works: Run setup script")
    print("   2. If CAR API token works: JWT generation is working!")
    print("   3. If neither works: Check authorization in Auth0 Dashboard")
    print("=" * 60)
