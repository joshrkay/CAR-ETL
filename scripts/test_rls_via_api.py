...
def test_rls_via_api():
    ...
    if response.status_code == 200:
        ...
        if user_tenant_id == tenant_a_id:
            print(f"   [OK] User A has correct tenant_id in auth context")
        else:
            print(f"   [FAIL] User A has wrong tenant_id")
    else:
    ...
    if response.status_code == 200:
        ...
        if user_tenant_id == tenant_b_id:
            print(f"   [OK] User B has correct tenant_id in auth context")
        else:
            print(f"   [FAIL] User B has wrong tenant_id")
    else:
        print(f"   [ERROR] Failed to get user context: {response.status_code}")
    ...
    try:
        ...
        if tenant_a_id in tenant_ids_a and tenant_b_id not in tenant_ids_a:
            print(f"   [OK] User A membership is isolated (Tenant A only)")
        else:
            print(f"   [FAIL] User A has access to wrong tenants")
        ...
        if tenant_b_id in tenant_ids_b and tenant_a_id not in tenant_ids_b:
            print(f"   [OK] User B membership is isolated (Tenant B only)")
        else:
            print(f"   [FAIL] User B has access to wrong tenants")
