...
CREATE POLICY "Service role can manage tenant_users" ON public.tenant_users
  FOR ALL USING (auth.role() = 'service_role');

-- Allow authenticated users to read their own tenant assignments
CREATE POLICY "Users can read their own tenant assignments" ON public.tenant_users
